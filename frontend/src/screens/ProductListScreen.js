...
import { PRODUCT_CREATE_RESET } from '../redux/types/productTypes'
  ...
  useEffect(() => {
    ...
    // }
    else if(createdProduct && successCreate){
      history.push(`/admin/product/${createdProduct._id}/edit`)
    }
    else{
  ...
  //because of successDelete here , product will refresh and gone from page
  ...
  const deleteHandeler = (id) => {
    if(window.confirm('Are you sure!')){
      ...
      if(errorDelete){
        errorDelete && <Message variant='danger'>{'Delete loading'+ errorDelete}</Message>
      }
    }
  ...
  const createProductHandeler = () => {
    ...
    if(loadingCreate) {
      ...
      errorCreate && <Message variant='danger'>{'Create loading'+ errorCreate}</Message>
     }
  }
